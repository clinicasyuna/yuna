# Copyright e Propriedade Intelectual - Sistema YUNA

## 📋 **Informações de Copyright**

**Nome do Software:** Sistema YUNA - Gerenciamento de Solicitações para Clínicas
**Versão:** 2.0.0
**Data de Criação:** 14 de novembro de 2024
**Última Atualização:** 15 de dezembro de 2025
**Autor/Proprietário:** Samuel dos Reis Lacerda Junior
**Empresa:** 55.004.442 SAMUEL DOS REIS LACERDA JUNIOR (MEI)

---

## 🏛️ **Declaração de Direitos Autorais**

```
Copyright © 2024-2025 Samuel dos Reis Lacerda Junior. Todos os direitos reservados.

Este software e sua documentação são propriedade intelectual protegida por 
direitos autorais. Nenhuma parte deste software pode ser reproduzida, 
distribuída, ou transmitida de qualquer forma ou por qualquer meio, 
incluindo fotocópia, gravação ou outros métodos eletrônicos ou mecânicos, 
sem a permissão prévia por escrito do proprietário, exceto no caso de 
breves citações incorporadas em análises críticas e certos outros 
usos não comerciais permitidos pela lei de direitos autorais.
```

---

## 🔐 **Componentes Protegidos**

### **Código Fonte:**
- Arquitetura de frontend JavaScript
- Integração Firebase/Firestore
- Sistema de autenticação e permissões
- Dashboard executivo com métricas avançadas
- Interface de acompanhantes de pacientes
- Sistema de alertas inteligentes
- Componentes PWA (Service Worker, Manifest)

### **Design e Interface:**
- Layout e design visual
- Fluxos de usuário (UX)
- Iconografia e elementos gráficos
- Esquema de cores e tipografia

### **Lógica de Negócio:**
- Algoritmos de cálculo de métricas
- Sistema de roles e permissões
- Fluxo de trabalho de solicitações
- Lógica de alertas e notificações

### **Documentação:**
- Manuais de usuário
- Documentação técnica
- Guias de implementação

---

## ⚖️ **Uso Permitido**

Este software é licenciado exclusivamente para uso comercial autorizado. 
Qualquer uso não autorizado é estritamente proibido e sujeito a 
penalidades legais.

Para obter licenças de uso, entre em contato:
- Email: samukajr82@gmail.com
- Telefone: +55 11 94586-4671
- GitHub: https://github.com/clinicasyuna/yuna

---

**Data de Criação deste Documento:** 14 de novembro de 2024
**Última Atualização:** 15 de dezembro de 2025